...
const SHEET_DEFS = {
  ...
  cookedHistory: { name: "調理履歴", headers: ["日付", "料理名", "レシピID", "人数", "食費", "使用食材", "手動追加"] },
...
//   ・「調理履歴」：料理名・人数・食費・使用食材(JSON文字列)・手動追加フラグを保持
//     （レシピが後から編集・削除されても、記録時点の内容がそのまま残るようにするため）
//   ・「設定」シート：月別食費予算の保存のためにシート自体を追加
